...
@section('content')
    ...
    <div class="mx-auto flex w-full max-w-[1200px] flex-col gap-6 px-4 py-10 md:gap-10 md:px-0 md:py-20">
            ...
            <form id="contact-form" method="POST" action="{{ route('send.contact') }}"
                ...
                <div class="flex flex-col gap-2  sm:gap-3">
                    ...
                    <textarea required name="mensaje" id="mensaje"
                        class="h-[150px] w-full border border-[#EEEEEE] pt-2 pl-3">{{ old('mensaje', $producto ?? '') }}</textarea>
                ...
                <div class="flex flex-col justify-end gap-3 ">
                    ...
                    @error('g-recaptcha-response') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    <button type="submit"
        ...
        <script src="https://www.google.com/recaptcha/api.js?render=6LdUqGorAAAAAP2dxLcNbentWlPL4MorCbS6e4b0
                                                "></script>
        <script>
            ...
            document.getElementById('contact-form').addEventListener('submit', function (event) {
                ...
                grecaptcha.ready(function () {
                    grecaptcha.execute('6LdUqGorAAAAAP2dxLcNbentWlPL4MorCbS6e4b0', { action: 'submit' }).then(function (token) {
                        // Asignar el token al campo oculto
                        document.getElementById('recaptcha-token').value = token;

                        // Enviar el formulario después de agregar el token
                        event.target.submit();
                    });
                });
            });
